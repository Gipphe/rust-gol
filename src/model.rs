fn count_neighbors(board: &Vec<Vec<bool>>, cell_y: i32, cell_x: i32) -> i32 {
	let rows: [Option<&Vec<bool>>; 3] = [
		board.get((cell_y - 1) as usize),
		board.get(cell_y as usize),
		board.get((cell_y + 1) as usize)
	];
	let mut count: i32 = 0;
	for x in -1..2 {
		...
		for i in 0..rows.len() {
			...
			let row: Option<&Vec<bool>> = rows[i];
			match row {
				Some(real_row) => {
					let foo: bool = false;
					let is_alive: &bool = real_row.get(real_x)
						.unwrap_or(&foo);
					if *is_alive {
						count += 1;
					}
...
pub fn step(old_gen: &Vec<Vec<bool>>) -> Vec<Vec<bool>> {
	let mut next_gen: Vec<Vec<bool>> = old_gen.clone();
	for y in 0..old_gen.len() {
		let row: &[bool] = &old_gen[y];
		for x in 0..row.len() {
			let cell: bool = row[x];
